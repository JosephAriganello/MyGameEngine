...
yaw(0.0f), pitch(0.0f), forwardVector(glm::vec3()), upVector(glm::vec3()), 
rightVector(glm::vec3()), worldUp(glm::vec3()), lights(std::vector<LightSource*>()){
	...
	perspective = glm::perspective(fieldOfView, Engine::GetInstance()->GetScreenSize().x /
		Engine::GetInstance()->GetScreenSize().y, nearPlane, farPlane);

	orthographic = glm::ortho(0.0f, Engine::GetInstance()->GetScreenSize().x, 0.0f,
		Engine::GetInstance()->GetScreenSize().y, -1.0f, 1.0f);
}
